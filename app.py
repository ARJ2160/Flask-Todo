...
@app.route("/update/<int:sno>", methods=['UPDATE', 'POST', 'GET'])
def update(sno):
  if request.method == 'POST':
    title = request.form["title"]
    desc = request.form["desc"]
    ...
    todo.title = title
    todo.desc = desc
    todo.date_created = db.Column(db.DateTime, default=datetime.utcnow)
    db.session.add(todo)
    db.session.commit()
    return redirect("/")
...
@app.route("/delete/<int:sno>", methods=['DELETE', 'POST'])
def delete(sno):
